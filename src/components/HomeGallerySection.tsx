  ...
  return (
    <section className="py-16 sm:py-24 bg-muted/30 border-y border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Reveal>
          <figure className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-md mb-4 sm:mb-5 group card-lift">
            <img
              ...
              loading="lazy"
              decoding="async"
              className="w-full aspect-[2.5/1] object-cover bg-muted/30 transition-transform duration-700 group-hover:scale-[1.03]"
            />
          </figure>
        </Reveal>

        <Stagger className="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
          {photos.map((p) => (
            <StaggerItem key={p.alt} className="break-inside-avoid">
              <figure className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-sm group card-lift">
                <img
                  src={p.src}
                  alt={p.alt}
                  width={800}
